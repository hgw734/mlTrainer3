#!/usr/bin/env python3
"""
Custom Patterns Model Implementations
"""
...
class PatternsModel:
    ...
    def __init__(self, pattern_window: int = 30):
        self.pattern_window = pattern_window
        self.is_fitted = False

    def fit(self, data: pd.Series) -> 'PatternsModel':
        if len(data) < self.pattern_window:
            raise ValueError(f"Insufficient data: {len(data)} < {self.pattern_window}")
        self.is_fitted = True
        return self
    ...
    def predict(self, data: pd.Series) -> pd.Series:
        ...
        for i in range(self.pattern_window, len(data)):
            window_data = data.iloc[i-self.pattern_window:i+1]

            # Calculate pattern feature (e.g., local min/max)
            if window_data.iloc[-1] == window_data.max():
                signals.iloc[i] = 1.0
            elif window_data.iloc[-1] == window_data.min():
                signals.iloc[i] = -1.0

        return signals

    def get_parameters(self) -> Dict[str, Any]:
        return {'pattern_window': self.pattern_window, 'is_fitted': self.is_fitted} 