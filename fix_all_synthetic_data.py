...
def remove_deterministic_methods(filepath: str):
    ...
    new_content = re.sub(r'self\._deterministic_(normal|uniform|randn|random)\([^)]*\)', 
                         '0.5  # Fixed value - was synthetic', new_content)
    
    # Remove np.random usage
    new_content = re.sub(r'np\.random\.seed\(\d+\)', '# Removed np.random.seed', new_content)
...
def fix_specific_files():
    ...
    if os.path.exists('custom/automl.py'):
        ...
        # Replace line 73
        if len(lines) > 73 and 'best_score = self._deterministic_uniform' in lines[73]:
