...
class SP500UniverseManager:
    ...
    def get_market_cap_data(self, symbols: List[str]) -> Dict[str, float]:
        ...
        try:
            ...
            logger.info(f"Retrieved market cap data for {len(market_caps)} stocks")
            return market_caps
    ...
    def calculate_weights(self, stocks: List[SP500Stock], market_caps: Dict[str, float]) -> List[SP500Stock]:
        ...
        for stock in stocks:
            if stock.symbol in market_caps:
                ...
                stock.weight = market_caps[stock.symbol] / total_market_cap if total_market_cap > 0 else 0
            else:
    ...
    def save_universe(self, universe: SP500Universe):
        ...
        try:
            ...
            logger.info(f"Saved S&P 500 universe data to {universe_file} and {csv_file}")
    ...
    def load_universe(self) -> Optional[SP500Universe]:
        ...
        try:
            ...
            stocks = [SP500Stock(**stock_data) for stock_data in data['stocks']]
    ...
    def get_current_universe(self) -> SP500Universe:
        ...
        if self.should_update_universe():
            ...
            if universe:
                ...
                logger.warning("Failed to create new universe, loading cached data")
            ...
            if self.universe_data is None:
                ...
                if self.universe_data is None:
                    logger.info("No cached data available, creating new universe...")
                    universe = self.create_universe()
                    if universe:
                        self.save_universe(universe)
                        self.universe_data = universe
    ...
    def get_top_stocks_by_market_cap(self, n: int = 50) -> List[SP500Stock]:
        ...
        sorted_stocks = sorted(universe.stocks, key=lambda x: x.market_cap or 0, reverse=True)
    ...
    def _get_next_update_date(self) -> str:
        ...
        # If we're past all months this year, next update is first month next year
        next_date = datetime(now.year + 1, self.update_months[0], self.update_days[0])
        return next_date.isoformat()
...
if __name__ == "__main__":
    ...
    if universe:
        ...
        for stock in top_stocks:
            print(f"  {stock.symbol}: {stock.company_name} (${stock.market_cap:,.0f})")
