...
class VirtualPortfolioManager:
    ...
    def __init__(self, initial_capital: float = 100000.0):
        ...
        self.positions: Dict[int, VirtualPosition] = {}  # recommendation_id -> position
        ...
        logger.info(f"Virtual Portfolio Manager initialized with ${initial_capital:,.2f}")
    ...
    def open_position(self, recommendation: Dict) -> Optional[VirtualPosition]:
        ...
        if not self.can_open_position():
            logger.warning("Cannot open new position - limit reached or insufficient cash")
            return None
        
        try:
            # Calculate position size
            shares = self.calculate_position_size(recommendation['entry_price'])
            ...
            logger.info(f"Opened virtual position: {shares} shares of {position.symbol} at ${position.entry_price:.2f}")
            return position
        ...
        for position in open_positions:
            if position.symbol in current_prices:
                ...
                if old_price != position.current_price:
                    logger.debug(f"Updated {position.symbol}: ${old_price:.2f} -> ${position.current_price:.2f}")
    ...
    def _check_exit_conditions(self, position: VirtualPosition):
        ...
        if position.current_price <= position.stop_loss:
            self.close_position(position.recommendation_id, ExitReason.STOP_LOSS)
            return
        
        # Check target
        if position.current_price >= position.target_price:
            self.close_position(position.recommendation_id, ExitReason.TARGET_HIT)
            return
    ...
    def close_position(self, recommendation_id: int, reason: ExitReason) -> Optional[VirtualPosition]:
        ...
        logger.info(f"Closed position: {position.symbol} at ${position.exit_price:.2f} "
    ...
    def get_portfolio_metrics(self) -> Dict:
        ...
        if closed:
            ...
            avg_loss = np.mean([p.profit_loss_pct for p in losses]) if losses else 0
    ...
    def _save_position_to_db(self, position: VirtualPosition):
        ...
        logger.debug(f"Would save position {position.recommendation_id} to database")
    
    def _update_position_in_db(self, position: VirtualPosition):
        """Update position in database"""
        # TODO: Implement database update
        logger.debug(f"Would update position {position.recommendation_id} in database")
    
    def _close_position_in_db(self, position: VirtualPosition):
        """Mark position as closed in database"""
        # TODO: Implement database close
        logger.debug(f"Would close position {position.recommendation_id} in database")
