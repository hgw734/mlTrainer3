...
# Load model registry
try:
    # We'll parse the models_config.py file to extract model info without
    # importing numpy
    with open("config/models_config.py", "r") as f:
        ...
        print(
            f"📊 Found {custom_count} models configured with library='custom'")
                                ...
                                for module_path, class_name in test_cases:
                                    try:
                                        ...
                                        if hasattr(module, class_name):
                                            print(
                                                f"✅ Found: {module_path}.{class_name}")
                                            implementations_found += 1
                                            else:
                                                print(
                                                    f"❌ Missing: {module_path}.{class_name}")
                                                implementations_missing += 1
                                                    ...
                                                    print(f"\n📊 Summary:")
                                                    print(
                                                        f"   - Implementation modules found: {len(found_modules)}")
                                                    print(
                                                        f"   - Implementation modules missing: {len(missing_modules)}")
                                                    print(
                                                        f"   - Model classes found: {implementations_found}")
                                                    print(
                                                        f"   - Model classes missing: {implementations_missing}")

                                                    if missing_modules or implementations_missing > 0:
                                                        print(
                                                            "\n⚠️  COMPLIANCE WARNING: Missing model implementations detected!")
                                                        print(
                                                            "The compliance system should prevent the trading system from starting.")
                                                        else:
                                                            print(
                                                                "\n✅ All checked implementations are present!")
                                                            print(
                                                                "Note: This is not a complete check - the actual compliance system does more.")
