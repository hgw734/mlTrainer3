...
def test_no_hardcoded_api_keys():
    ...
    r'os\.getenv\(["\'][^"\']+["\']\s*,\s*["\'][a-zA-Z0-9]{10,}["\']',  # Default values
            ...
            try:
                with open(file_path, "r", encoding="utf-8") as f:
                    ...
                    for pattern in hardcoded_patterns:
                        ...
                        if matches:
                            violations.append({"file": str(file_path), "pattern": pattern, "matches": matches})
                            except Exception:
                            ...
                            def test_config_loader_initialization():
                                ...
                                try:
                                    ...
                                    assert loader.env in ["development", "staging", "production"]
                                    except Exception as e:
                                        pytest.fail(f"Config loader failed to initialize: {e}")
                                        ...
                                        def test_config_requires_env_vars():
                                            ...
                                            with pytest.raises(ConfigurationError) as exc_info:
                                                ...
                                                assert "environment variable required" in str(exc_info.value)
                                                ...
                                                def test_config_get_values():
                                                    ...
                                                    rate_limit = get_config("api.polygon.rate_limit")
                                                    assert rate_limit == 100  # Default value

                                                    # Test getting with default
                                                    missing = get_config("api.polygon.missing_key", "default_value")
                                                    ...
                                                    def test_config_compliance_validation():
                                                        ...
                                                        assert loader.validate_against_compliance("models.random_forest.n_estimators", 100) == True

                                                        # Should reject based on compliance rules
                                                        # (This assumes compliance rules exist for the path)
                                                        # The actual test depends on your compliance configuration
                                                        ...
                                                        def test_all_configs_accessible():
                                                            ...
                                                            # These should all return non-None values
                                                            assert get_config("api") is not None
                                                            assert get_config("models") is not None
                                                            assert get_config("ai") is not None
                                                            assert get_config("compliance") is not None


                                                            if __name__ == "__main__":
                                                                # Run tests
                                                                print("🧪 Testing Phase 1: Configuration System")
                                                                print(("=" * 50))
