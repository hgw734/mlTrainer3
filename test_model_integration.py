...
def test_ml_models():
    ...
    try:
        ...
        for category, info in list(summary["categories"].items()):
            ...
            try:
                result = manager.train_model("linear_regression", symbol="AAPL", data_source="polygon")

                if result.compliance_status == "approved":
                    print(f"✓ Model trained successfully!")
                    print(f"  R² Score: {result.performance_metrics.get('r2_score', 0):.4f}")
                    print(f"  Training time: {result.training_time:.2f}s")
                    else:
                        print(f"✗ Model training failed compliance: {result.compliance_status}")
                            ...
                            for model_id in manager.get_available_models()[:5]:
                                ...
                                return True
                                    ...
                                    print(f"✗ ML Model Manager test failed: {e}")
                                    logger.error(f"ML test error: {e}", exc_info=True)
                                    return False
                                    ...
                                    def test_financial_models():
                                        ...
                                        try:
                                            ...
                                            print(f"✓ Financial Model Manager initialized")
                                            ...
                                            for model_id in models:
                                                ...
                                                try:
                                                    ...
                                                    if result.compliance_status == "approved":
                                                        print(f"✓ Black-Scholes calculated successfully!")
                                                        print(f"  Option Price: ${result.option_price:.2f}")
                                                        print(f"  Delta: {result.greeks['delta']:.4f}")
                                                        print(f"  Gamma: {result.greeks['gamma']:.4f}")
                                                        else:
                                                            print(f"✗ Calculation failed compliance")

                                                            except Exception as e:
                                                                print(f"✗ Black-Scholes test failed: {str(e)[:100]}")
                                                                ...
                                                                try:
                                                                    ...
                                                                    result = manager.run_model("value_at_risk", returns=returns, confidence_level=0.95, method="historical")

                                                                    if result.compliance_status == "approved":
                                                                        print(f"✓ VaR calculated successfully!")
                                                                        print(f"  VaR (95%): {result.risk_metrics['var']:.4f}")
                                                                        print(f"  Expected Shortfall: {result.risk_metrics['expected_shortfall']:.4f}")
                                                                        else:
                                                                            print(f"✗ VaR calculation failed compliance")

                                                                            except Exception as e:
                                                                                print(f"✗ VaR test failed: {str(e)[:100]}")

                                                                                return True

                                                                                except Exception as e:
                                                                                    print(f"✗ Financial Model Manager test failed: {e}")
                                                                                    logger.error(f"Financial test error: {e}", exc_info=True)
                                                                                    return False
                                                                                    ...
                                                                                    def test_model_integration():
                                                                                        ...
                                                                                        try:
                                                                                            ...
                                                                                            print(f"✓ Model Integration initialized")
                                                                                            ...
                                                                                            for response in test_responses:
                                                                                                request = integration.parse_model_request(response)
                                                                                                if request["model_id"] or request["action"]:
                                                                                                    print(f"✓ Parsed: {response[:50]}...")
                                                                                                    print(f"  Type: {request['type']}, Action: {request['action']}, Model: {request['model_id']}")
                                                                                                        ...
                                                                                                        for rec in recommendations:
                                                                                                            ...
                                                                                                            return True
                                                                                                                ...
                                                                                                                def main():
                                                                                                                    ...
                                                                                                                    print(f"Financial Models: {'✅ PASS' if financial_ok else '❌ FAIL'}")
                                                                                                                    ...
                                                                                                                    if ml_ok and financial_ok and integration_ok:
                                                                                                                        ...
                                                                                                                        print("\nYou can now:")
                                                                                                                        print("1. Ask Claude to train ML models (e.g., 'Train random_forest_100 on AAPL')")
                                                                                                                        print("2. Run financial models (e.g., 'Calculate Black-Scholes for...')")
                                                                                                                        print("3. Optimize portfolios (e.g., 'Optimize portfolio with AAPL, MSFT, GOOGL')")
                                                                                                                        print("4. Assess risk (e.g., 'Calculate VaR for my portfolio')")
                                                                                                                        else:
