...
class DualMomentumStrategy:
    ...
    def create_features_7_12(self, ticker_data, macro_data):
        ...
        # Volatility
        df['realized_vol_10d'] = df['close'].pct_change().rolling(10).std() * \
            np.sqrt(252)
        df['atr_10'] = self._calculate_atr(df, 10)

        # Intraday features from minute data
        if ticker_data['minute'] is not None and len(
                ticker_data['minute']) > 0:
            df['intraday_vol'] = self._calculate_intraday_volatility(
                ticker_data['minute'])
            df['vwap_ratio'] = self._calculate_vwap_ratio(
                ticker_data['minute'], df)

        # Macro features
        df['vix_level'] = macro_data['vix']['close'].reindex(
        ...
        # Market regime
        df['vol_regime'] = df['realized_vol_10d'] / \
            df['realized_vol_10d'].rolling(60).mean()
    ...
    def create_features_50_70(self, ticker_data, macro_data):
        ...
        df['momentum_quality'] = self._calculate_momentum_quality(
            df['close'], 60)

        # Volatility and risk
        df['realized_vol_60d'] = df['close'].pct_change().rolling(60).std() * \
            np.sqrt(252)
        df['downside_vol_60d'] = self._calculate_downside_volatility(
            df['close'], 60)
        df['max_dd_60d'] = self._calculate_rolling_max_drawdown(
            df['close'], 60)

        # Relative strength
        spy_returns = self.polygon.get_ohlcv_data(
            'SPY', df.index[0], df.index[-1])
        spy_returns = spy_returns['close'].pct_change(
            60).reindex(df.index, method='ffill')
        df['relative_strength'] = df['return_60d'] / spy_returns

        # Macro features (more important for longer timeframes)
        df['gdp_growth'] = macro_data['gdp_growth'].reindex(
            df.index, method='ffill')
        df['unemployment_trend'] = macro_data['unemployment'].pct_change(
            3).reindex(df.index, method='ffill')
        df['credit_spread'] = macro_data['credit_spread'].reindex(
            df.index, method='ffill')
    ...
    def _calculate_intraday_volatility(self, minute_data):
        ...
        daily_vol = minute_returns.groupby(
            minute_data.index.date).std() * np.sqrt(390)
    ...
    def _calculate_vwap_ratio(self, minute_data, daily_data):
        ...
        minute_data['dollar_volume'] = minute_data['close'] * \
            minute_data['volume']
        daily_vwap = (
            minute_data.groupby(
                minute_data.index.date)['dollar_volume'].sum() /
            minute_data.groupby(
                minute_data.index.date)['volume'].sum())
    ...
    def train_models(self, train_data, train_start, train_end):
        ...
        for ticker in self.universe:
            # Create features
            features_7_12 = self.create_features_7_12(
                train_data[ticker], train_data['macro'])
            features_50_70 = self.create_features_50_70(
                train_data[ticker], train_data['macro'])
            ...
            features_7_12['target'] = features_7_12['close'].pct_change(
                10).shift(-10)
            features_50_70['target'] = features_50_70['close'].pct_change(
                60).shift(-60)
    ...
    def generate_signals(self, current_data):
        ...
        for ticker in self.universe:
            # Create features
            features_7_12 = self.create_features_7_12(
                current_data[ticker], current_data['macro'])
            features_50_70 = self.create_features_50_70(
                current_data[ticker], current_data['macro'])
            ...
            # Make predictions
            pred_7_12 = self._ensemble_predict(
                self.models_7_12, latest_7_12, self.scaler_7_12)
            pred_50_70 = self._ensemble_predict(
                self.models_50_70, latest_50_70, self.scaler_50_70)
