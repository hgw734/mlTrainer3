...
class ComponentInfo:
    ...
    def _calculate_hash(self) -> str:
        ...
        try:
            ...
            logger.warning(f"Could not calculate hash for {self.module_path}: {e}")
        return ""
...
class ModularIntegrationSystem:
            ...
            try:
                with open(self.registry_path, 'r') as f:
                    ...
                    for key, component_data in data.items():
                        self.components[key] = ComponentInfo.from_dict(component_data)
    ...
    def _save_registry(self):
        ...
        try:
            with open(self.registry_path, 'w') as f:
                data = {key: component.to_dict() for key, component in self.components.items()}
    ...
    def _discover_components(self):
        ...
        for py_file in project_root.rglob("*.py"):
            ...
            status = ComponentStatus.PROTECTED if str(relative_path) in self.protected_modules else ComponentStatus.ACTIVE
            ...
            if component_name not in self.components:
                ...
                logger.info(f"Discovered new component: {component_name} ({component_type.value})")
            else:
    ...
    def register_component(self, name: str, component_type: ComponentType, module_path: str,
                          class_name: Optional[str] = None, function_name: Optional[str] = None,
                          dependencies: List[str] = None, metadata: Dict[str, Any] = None) -> ComponentInfo:
        """Register a new component"""
        
        # Check if component is protected
        if module_path in self.protected_modules:
            logger.warning(f"Attempted to register protected component: {name}")
            return None
    ...
    def load_component(self, name: str) -> Optional[Any]:
        ...
        try:
            # Import module
            spec = importlib.util.spec_from_file_location(component.name, component.module_path)
    ...
    def get_system_summary(self) -> Dict[str, Any]:
        ...
        # Count by type
        for component_type in ComponentType:
            summary['by_type'][component_type.value] = len(self.get_components_by_type(component_type))
...
def register_component(name: str, component_type: ComponentType, module_path: str, **kwargs) -> ComponentInfo:
    """Convenience function to register a component"""
    return modular_system.register_component(name, component_type, module_path, **kwargs)
...
def get_components_by_type(component_type: ComponentType) -> List[ComponentInfo]:
