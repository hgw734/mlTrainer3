...
# Page configuration
st.set_page_config(page_title="mlTrainer Monitor", page_icon="📊", layout="wide", initial_sidebar_state="expanded")
...
def get_system_metrics():
    ...
    def get_api_status():
        ...
        try:
            ...
            status["polygon"] = {"status": "ERROR", "message": str(e), "metrics": None}

        # Check FRED
        try:
            fred_connector = get_fred_connector()
            gdp = fred_connector.get_series_data("GDP", start_date="2024-01-01")
            status["fred"] = {
    ...
    def get_chat_activity():
        ...
        return {"total_messages": 0, "last_24h": 0, "hourly_counts": {i: 0 for i in range(24)}}
        ...
        with st.sidebar:
            ...
            with tab1:
                ...
                with col4:
                    ...
                    st.markdown("<p class='status-ok'>Active</p>", unsafe_allow_html=True)
            ...
            with tab2:
                ...
                with col1:
                    ...
                    if "polygon" in api_status:
                        ...
                        st.markdown(f"<p class='status-{status.lower()} big-font'>{status}</p>", unsafe_allow_html=True)
                        ...
                        if api_status["polygon"].get("metrics"):
                            ...
                            st.metric("Dropout Rate", f"{metrics['dropout_rate']:.1%}")
                                ...
                                if "fred" in api_status:
                                    ...
                                    with col3:
                                        ...
                                        if "claude" in api_status:
                                            ...
                                            st.markdown(f"<p class='status-{status.lower()} big-font'>{status}</p>", unsafe_allow_html=True)
                                            st.caption(api_status["claude"]["message"])

                                            # Rate limiter status
                                            if "polygon" in api_status and api_status["polygon"].get("metrics"):
                                                st.divider()
                                                st.subheader("📊 Polygon Rate Limiter Status")
                                                ...
                                                with col1:
                                                    ...
                                                    with col2:
                                                        st.metric("Circuit Breaker", "OPEN" if metrics["circuit_open"] else "CLOSED")
                                                        with col3:
                                                            st.metric("Rate Limit Active", "YES" if metrics["rate_limit_active"] else "NO")
                                                            with col4:
                                                                st.metric("Avg Response", f"{metrics['avg_response_time']:.2f}s")

                                                # Tab 3: ML Trials
                                                with tab3:
                                                    st.header("Machine Learning Trials")
                                                    ...
                                                    if trials:
                                                        ...
                                                        with col1:
                                                            ...
                                                            with col2:
                                                                successful = sum(1 for t in trials if t.get("status") == "completed")
                                                                st.metric("Successful", successful)

                                                                with col3:
                                                                    avg_accuracy = sum(t.get("accuracy", 0) for t in trials if t.get("accuracy")) / max(
                                                                        1, len([t for t in trials if t.get("accuracy")])
                                                                    )
                                                                    st.metric("Avg Accuracy", f"{avg_accuracy:.2%}")

                                                                    with col4:
                                                                        avg_profit = sum(t.get("profit_loss", 0) for t in trials if t.get("profit_loss")) / max(
                                                                            1, len([t for t in trials if t.get("profit_loss")])
                                                                        )
                                                                        st.metric("Avg Profit/Loss", f"{avg_profit:+.2f}%")
                                                                        ...
                                                                        table_data = []
                                                                        for trial in trials[-10:]:  # Last 10 trials
                                                                            table_data.append(
                                                                                {
                                                        ...
                                                        st.info("No trial history available yet")
                                                    ...
                                                    with tab4:
                                                        ...
                                                        with col1:
                                                            st.metric("Total Messages", activity["total_messages"])

                                                            with col2:
                                                                st.metric("Last 24 Hours", activity["last_24h"])
                                                                ...
                                                                with col3:
                                                                    ...
                                                                    st.subheader("Messages by Hour (Last 24h)")

                                                                    hours = list(range(24))
                                                                    counts = [activity["hourly_counts"][h] for h in hours]

                                                                    fig = px.bar(x=hours, y=counts, labels={"x": "Hour", "y": "Messages"}, title="Chat Activity Pattern")
                                                    ...
                                                    with tab5:
                                                        ...
                                                        try:
                                                            ...
                                                            col_per_row = 3
                                                            rows = (len(symbols) + col_per_row - 1) // col_per_row

                                                            for row in range(rows):
                                                                cols = st.columns(col_per_row)
                                                                for col_idx in range(col_per_row):
