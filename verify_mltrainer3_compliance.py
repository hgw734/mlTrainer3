...
def verify_compliance_gateway():
    ...
    try:
        ...
        valid_source = gateway.verify_data_source("polygon_api", "/v2/aggs/ticker/AAPL/range/1/day/2023-01-01/2023-12-31")
        ...
        invalid_source = gateway.verify_data_source("fake_api", "/fake/endpoint")
...
def scan_for_synthetic_data():
    ...
    if violations_by_file:
        ...
        for filepath, violations in violations_by_file.items():
            print(f"\n  {filepath}:")
            for line_num, pattern, line in violations[:3]:  # Show first 3 violations per file
...
def verify_mltrainer_claude_integration():
    ...
    try:
        ...
        if "synthetic data" in response.lower() and "not allowed" in response.lower():
            ...
            print(f"{YELLOW}⚠ Claude response to synthetic data request unclear{RESET}")
