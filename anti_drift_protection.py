...
class AntiDriftProtection:
        ...
        if self.config_path.exists():
            try:
                with open(self.config_path, 'r') as f:
                    ...
                    # Load fingerprints
                    for path, fingerprint_data in data.get('fingerprints', {}).items():
                        self.fingerprints[path] = FileFingerprint.from_dict(fingerprint_data)
                    
                    # Load protected files
                    self.protected_files = set(data.get('protected_files', []))
                    
                    logger.info(f"Loaded {len(self.fingerprints)} file fingerprints")
    ...
    def _save_config(self):
        ...
        try:
            data = {
                'fingerprints': {path: fp.to_dict() for path, fp in self.fingerprints.items()},
    ...
    def get_file_info(self, file_path: str) -> Optional[Tuple[int, datetime, int]]:
    ...
    def create_fingerprint(self, file_path: str, is_protected: bool = False) -> Optional[FileFingerprint]:
    ...
    def restore_file(self, file_path: str) -> bool:
        ...
        fingerprint = self.fingerprints[file_path]
        if not fingerprint.backup_path or not Path(fingerprint.backup_path).exists():
    ...
    def _should_auto_restore(self, file_path: str) -> bool:
        ...
        if file_path in self.fingerprints:
            return self.fingerprints[file_path].metadata.get('auto_restore', False)
    ...
    def add_file_to_monitoring(self, file_path: str, auto_restore: bool = False):
