                    ...
                    def __init__(self, bot_token: Optional[str] = None, chat_id: Optional[str] = None):
                        ...
                        if not self.bot_token or not self.chat_id:
                            ...
                            if not self._test_connection():
                                raise ValueError("Cannot connect to Telegram bot")
                                ...
                                logger.info("Telegram notifier ready for notifications")
                                ...
                                def _test_connection(self) -> bool:
                                    ...
                                    try:
                                        ...
                                        if response.status_code == 200:
                                            data = response.json()
                                            if data.get("ok"):
                                                logger.info(f"Connected to bot: @{data['result']['username']}")
                                                return True
                                                return False
                                                except Exception as e:
                                                    logger.error(f"Connection production failed: {e}")
                                                    return False

                                                    def _format_message(self, msg: TelegramMessage) -> str:
                                                        """Format message for Telegram"""
                                                        lines = [f"{msg.type.value} *{msg.title}*", "", msg.message]

                                                        if msg.details:
                                                            lines.append("")
                                                            lines.append("📋 *Details:*")
                                                            for key, value in list(msg.details.items()):
                                                                # Format key nicely
                                                                ...
                                                                if msg.timestamp:
                                                                    ...
                                                                    def send_message(self, msg: TelegramMessage) -> bool:
                                                                        """Send message via Telegram"""
                                                                        try:
                                                                            text = self._format_message(msg)

                                                                            data = {"chat_id": self.chat_id, "text": text, "parse_mode": "Markdown", "disable_web_page_preview": True}
                                                                            ...
                                                                            if response.status_code == 200:
                                                                                ...
                                                                                if result.get("ok"):
                                                                                    logger.info(f"Message sent: {msg.type.name} - {msg.title}")
                                                                                    return True
                                                                                    else:
                                                                                        ...
                                                                                        return False
                                                                                            ...
                                                                                            return False
                                                                                                ...
                                                                                                def send_trial_start(self, symbol: str, model: str, parameters: Dict):
                                                                                                    ...
                                                                                                    def send_trial_complete(self, symbol: str, model: str, results: Dict):
                                                                                                        ...
                                                                                                        def send_goal_update(self, goal: str, status: str):
                                                                                                            ...
                                                                                                            def send_market_alert(self, alert_type: str, details: Dict):
                                                                                                                ...
                                                                                                                def send_error(self, error_type: str, error_message: str, context: Optional[Dict] = None):
                                                                                                                    """Send error notification"""
                                                                                                                    msg = TelegramMessage(
                                                                                                                    type=NotificationType.ERROR, title=f"Error: {error_type}", message=error_message, details=context
                                                                                                                    )
                                                                                                                    return self.send_message(msg)
                                                                                                                    ...
                                                                                                                    def send_system_status(self, status: Dict):
                                                                                                                        ...
                                                                                                                        details: Optional[Dict] = None,
                                                                                                                            ...
                                                                                                                            msg = TelegramMessage(type=notification_type, title=title, message=message, details=details)
                                                                                                                            return self.send_message(msg)
                                                                                                                                ...
                                                                                                                                if _telegram_notifier is None:
                                                                                                                                    try:
                                                                                                                                        _telegram_notifier = TelegramNotifier()
                                                                                                                                            ...
                                                                                                                                            def notify_trial_start(symbol: str, model: str, parameters: Dict):
                                                                                                                                                """Notify trial start (convenience function)"""
                                                                                                                                                notifier = get_telegram_notifier()
                                                                                                                                                if notifier:
                                                                                                                                                    return notifier.send_trial_start(symbol, model, parameters)
                                                                                                                                                    return False


                                                                                                                                                    def notify_trial_complete(symbol: str, model: str, results: Dict):
                                                                                                                                                        """Notify trial completion (convenience function)"""
                                                                                                                                                        notifier = get_telegram_notifier()
                                                                                                                                                        if notifier:
                                                                                                                                                            return notifier.send_trial_complete(symbol, model, results)
                                                                                                                                                            return False


                                                                                                                                                            def notify_error(error_type: str, error_message: str, context: Optional[Dict] = None):
                                                                                                                                                                """Notify error (convenience function)"""
                                                                                                                                                                notifier = get_telegram_notifier()
                                                                                                                                                                if notifier:
                                                                                                                                                                    return notifier.send_error(error_type, error_message, context)
                                                                                                                                                                    return False
                                                                                                                                                                    ...
                                                                                                                                                                    def send_notification(message: str) -> bool:
                                                                                                                                                                        ...
                                                                                                                                                                        try:
                                                                                                                                                                            ...
                                                                                                                                                                            msg = TelegramMessage(type=NotificationType.INFO, title="mlTrainer Notification", message=message)
