...
class AgentProfile:
    ...
    def _update_capabilities(self):
        ...
        if self.trust_level in [TrustLevel.TRUSTED, TrustLevel.TRUSTED_PARTNER]:
            self.capabilities.update([
                'modify_files',
                'create_files',
                'run_commands',
    ...
    def add_action(self, action: TrustAction):
        ...
        self.current_score = max(0, min(100, self.current_score + action.score_change))
        self.last_updated = datetime.now()
        
        self._update_trust_level()
        self._update_capabilities()
        
        logger.info(f"Agent {self.agent_id}: {action.description} ({action.score_change:+d}) -> {self.current_score} ({self.trust_level.value})")
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'AgentProfile':
        ...
        profile = cls(
            ...
            actions=[TrustAction.from_dict(action_data) for action_data in data.get('actions', [])],
...
class AgentCodeOfConduct:
            ...
            try:
                with open(self.data_path, 'r') as f:
                    ...
                    for agent_id, profile_data in data.get('agents', {}).items():
                        self.agents[agent_id] = AgentProfile.from_dict(profile_data)
    ...
    def _save_agent_data(self):
        ...
        try:
            data = {
                ...
                'agents': {agent_id: profile.to_dict() for agent_id, profile in self.agents.items()}
            }
    ...
    def record_action(self, agent_id: str, action_type: ActionType, description: str, context: Dict[str, Any] = None):
    ...
    def check_file_access(self, agent_id: str, file_path: str, operation: str) -> bool:
        """Check if agent can access/modify a file"""
        agent = self.get_or_create_agent(agent_id)
        
        # Check if file is protected
        if file_path in self.protected_modules:
            if agent.trust_level not in [TrustLevel.TRUSTED, TrustLevel.TRUSTED_PARTNER]:
                logger.warning(f"Agent {agent_id} (trust: {agent.trust_level.value}) attempted to access protected file: {file_path}")
                return False
    ...
    def record_helpful_behavior(self, agent_id: str, description: str, context: Dict[str, Any] = None):
        """Record helpful behavior"""
        return self.record_action(agent_id, ActionType.HELPFUL_BEHAVIOR, description, context)
    
    def record_honest_limitation(self, agent_id: str, description: str, context: Dict[str, Any] = None):
        """Record honest limitation admission"""
        return self.record_action(agent_id, ActionType.HONEST_LIMITATION, description, context)
    
    def record_successful_implementation(self, agent_id: str, description: str, context: Dict[str, Any] = None):
        """Record successful implementation"""
        return self.record_action(agent_id, ActionType.SUCCESSFUL_IMPLEMENTATION, description, context)
    
    def record_false_claim(self, agent_id: str, description: str, context: Dict[str, Any] = None):
        """Record false claim or deception"""
        return self.record_action(agent_id, ActionType.FALSE_CLAIM, description, context)
    
    def record_destructive_action(self, agent_id: str, description: str, context: Dict[str, Any] = None):
        """Record destructive or harmful action"""
        return self.record_action(agent_id, ActionType.DESTRUCTIVE_ACTION, description, context)
    
    def record_compliance_violation(self, agent_id: str, description: str, context: Dict[str, Any] = None):
        """Record compliance violation (MOST SEVERE)"""
        return self.record_action(agent_id, ActionType.COMPLIANCE_VIOLATION, description, context)
    
    def record_security_violation(self, agent_id: str, description: str, context: Dict[str, Any] = None):
        """Record security violation"""
        return self.record_action(agent_id, ActionType.SECURITY_VIOLATION, description, context)
    ...
    def get_system_summary(self) -> Dict[str, Any]:
        ...
        for agent in self.agents.values():
            ...
            if (datetime.now() - agent.last_updated).total_seconds() < 3600:  # Last hour
...
def record_helpful_behavior(agent_id: str, description: str, context: Dict[str, Any] = None):
    """Record helpful behavior"""
    return code_of_conduct.record_helpful_behavior(agent_id, description, context)

def record_honest_limitation(agent_id: str, description: str, context: Dict[str, Any] = None):
    """Record honest limitation admission"""
    return code_of_conduct.record_honest_limitation(agent_id, description, context)

def record_successful_implementation(agent_id: str, description: str, context: Dict[str, Any] = None):
    """Record successful implementation"""
    return code_of_conduct.record_successful_implementation(agent_id, description, context)

def record_false_claim(agent_id: str, description: str, context: Dict[str, Any] = None):
    """Record false claim or deception"""
    return code_of_conduct.record_false_claim(agent_id, description, context)

def record_compliance_violation(agent_id: str, description: str, context: Dict[str, Any] = None):
    """Record compliance violation (MOST SEVERE)"""
    return code_of_conduct.record_compliance_violation(agent_id, description, context)
...
if __name__ == "__main__":
    ...
    # Record helpful behavior
    score, level = record_helpful_behavior(agent_id, "Successfully implemented data lineage system")
    print(f"Helpful behavior: {score} ({level.value})")
    
    # Record honest limitation
    score, level = record_honest_limitation(agent_id, "Admitted uncertainty about specific implementation details")
    print(f"Honest limitation: {score} ({level.value})")
    
    # Record false claim
    score, level = record_false_claim(agent_id, "Claimed to implement non-existent feature")
