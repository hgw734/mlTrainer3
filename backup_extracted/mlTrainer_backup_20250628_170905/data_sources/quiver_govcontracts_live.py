...
def fetch_live_gov_contracts() -> pd.DataFrame:
    """
    Fetch the latest live government contracts from QuiverQuant.
    """
    ...
    try:
        logger.info("🏛️ Fetching live government contracts from QuiverQuant")
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        data = response.json()
        return pd.DataFrame(data)
    except Exception as e:
        logger.error(f"❌ Failed to fetch live government contracts: {e}")
        return pd.DataFrame()
