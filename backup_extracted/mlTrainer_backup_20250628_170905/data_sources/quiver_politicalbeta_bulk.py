...
def fetch_bulk_political_beta() -> pd.DataFrame:
    """
    Fetch bulk political beta data from QuiverQuant.
    """
    api_key = load_quiver_api_key()
    url = "https://api.quiverquant.com/beta/bulk/politicalbeta"
    headers = {
        "Authorization": f"Bearer {api_key}"
    }

    try:
        logger.info("📦 Fetching bulk political beta data from QuiverQuant")
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        data = response.json()
        return pd.DataFrame(data)
    except Exception as e:
        logger.error(f"❌ Failed to fetch bulk political beta data: {e}")
        return pd.DataFrame()
