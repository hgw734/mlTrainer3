...
def fetch_live_senate_trading() -> pd.DataFrame:
    """
    Fetch live Senate stock trading activity from QuiverQuant.
    """
    ...
    try:
        logger.info("🏛️ Fetching live Senate trading activity from QuiverQuant")
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        data = response.json()
        return pd.DataFrame(data)
    except Exception as e:
        logger.error(f"❌ Failed to fetch live Senate trading data: {e}")
        return pd.DataFrame()
