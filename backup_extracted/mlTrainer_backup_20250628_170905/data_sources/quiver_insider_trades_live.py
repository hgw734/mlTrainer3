...
def fetch_live_insider_trades() -> pd.DataFrame:
    """
    Fetch the latest corporate insider trades from QuiverQuant.
    """
    ...
    try:
        logger.info("🏢 Fetching live insider trades from QuiverQuant")
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        data = response.json()
        return pd.DataFrame(data)
    except Exception as e:
        logger.error(f"❌ Failed to fetch live insider trading data: {e}")
        return pd.DataFrame()
