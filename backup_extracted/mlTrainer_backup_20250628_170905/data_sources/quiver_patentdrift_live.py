...
def fetch_live_patent_drift() -> pd.DataFrame:
    """
    Fetch live patent drift data from QuiverQuant.
    """
    ...
    try:
        logger.info("📄 Fetching live patent drift data from QuiverQuant")
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        data = response.json()
        return pd.DataFrame(data)
    except Exception as e:
        logger.error(f"❌ Failed to fetch patent drift data: {e}")
        return pd.DataFrame()
