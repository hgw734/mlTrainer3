...
def fetch_all_government_contracts() -> pd.DataFrame:
    """
    Fetch all available historical federal government contracts (not filtered by ticker).
    """
    api_key = load_quiver_api_key()
    url = "https://api.quiverquant.com/beta/historical/govcontractsall"
    headers = {
        "Authorization": f"Bearer {api_key}"
    }

    try:
        logger.info("📄 Fetching ALL government contracts from QuiverQuant")
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        data = response.json()
        return pd.DataFrame(data)
    except Exception as e:
        logger.error(f"❌ Error fetching all government contracts: {e}")
        return pd.DataFrame()
