...
def fetch_live_etf_holdings() -> pd.DataFrame:
    """
    Fetch the latest live ETF holdings from QuiverQuant.
    """
    api_key = load_quiver_api_key()
    url = "https://api.quiverquant.com/beta/live/etfholdings"
    headers = {
        "Authorization": f"Bearer {api_key}"
    }

    try:
        logger.info("📈 Fetching live ETF holdings from QuiverQuant")
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        data = response.json()
        return pd.DataFrame(data)
    except Exception as e:
        logger.error(f"❌ Failed to fetch live ETF holdings: {e}")
        return pd.DataFrame()
