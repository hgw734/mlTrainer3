...
def fetch_patent_filings() -> pd.DataFrame:
    """
    Fetch recent historical U.S. patent filings related to public companies
    """
    api_key = load_quiver_api_key()
    url = "https://api.quiverquant.com/beta/historical/allpatents"
    headers = {
        "Authorization": f"Bearer {api_key}"
    }

    try:
        logger.info("🧬 Fetching patent filings data from QuiverQuant")
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        data = response.json()
        return pd.DataFrame(data)
    except Exception as e:
        logger.error(f"❌ Failed to fetch patent filings: {e}")
        return pd.DataFrame()
