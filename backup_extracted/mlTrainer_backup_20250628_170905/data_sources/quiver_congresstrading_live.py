...
def fetch_live_congress_trades() -> pd.DataFrame:
    """
    Fetch the latest live congressional trades from QuiverQuant.
    """
    ...
    try:
        logger.info("🏛️ Fetching live congressional trades from QuiverQuant")
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        data = response.json()
        return pd.DataFrame(data)
    except Exception as e:
        logger.error(f"❌ Failed to fetch live congressional trades: {e}")
        return pd.DataFrame()
