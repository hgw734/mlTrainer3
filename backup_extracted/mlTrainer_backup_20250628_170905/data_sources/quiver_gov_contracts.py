...
def fetch_government_contracts(ticker: str) -> pd.DataFrame:
    """
    ...
    try:
        logger.info(f"📄 Fetching government contracts for {ticker.upper()}")
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        data = response.json()
        return pd.DataFrame(data)
    except Exception as e:
        logger.error(f"❌ Error fetching government contracts for {ticker}: {e}")
        return pd.DataFrame()
