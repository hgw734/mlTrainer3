...
def fetch_live_patent_momentum() -> pd.DataFrame:
    """
    Fetch live patent momentum scores from QuiverQuant.
    """
    ...
    try:
        logger.info("🚀 Fetching live patent momentum data from QuiverQuant")
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        data = response.json()
        return pd.DataFrame(data)
    except Exception as e:
        logger.error(f"❌ Failed to fetch patent momentum data: {e}")
        return pd.DataFrame()
