...
def fetch_political_beta(symbol: str) -> pd.DataFrame:
    """
    Fetch historical political beta exposure data for a given ticker.
    """
    api_key = load_quiver_api_key()
    url = f"https://api.quiverquant.com/beta/historical/politicalbeta/{symbol.upper()}"
    headers = {
        "Authorization": f"Bearer {api_key}"
    }

    try:
        logger.info(f"🏛️ Fetching political beta data for {symbol}")
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        data = response.json()
        return pd.DataFrame(data)
    except Exception as e:
        logger.error(f"❌ Error fetching political beta data for {symbol}: {e}")
        return pd.DataFrame()
