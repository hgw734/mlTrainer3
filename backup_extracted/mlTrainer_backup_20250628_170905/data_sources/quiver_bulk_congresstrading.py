...
def fetch_bulk_congress_trading() -> pd.DataFrame:
    """
    Fetch the full congressional trading dataset (bulk) from QuiverQuant (requires subscription).
    """
    ...
    try:
        logger.info("📥 Fetching full bulk congressional trading dataset from QuiverQuant (subscription access)")
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        data = response.json()
        return pd.DataFrame(data)
    except Exception as e:
        logger.error(f"❌ Failed to fetch bulk congressional trading data: {e}")
        return pd.DataFrame()
