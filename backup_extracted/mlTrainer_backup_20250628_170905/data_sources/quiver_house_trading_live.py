...
def fetch_live_house_trading() -> pd.DataFrame:
    """
    Fetch the latest House of Representatives trading activity from QuiverQuant.
    """
    ...
    try:
        logger.info("🏛️ Fetching live House trading activity from QuiverQuant")
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        data = response.json()
        return pd.DataFrame(data)
    except Exception as e:
        logger.error(f"❌ Failed to fetch live House trading data: {e}")
        return pd.DataFrame()
