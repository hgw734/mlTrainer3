...
def fetch_live_patents() -> pd.DataFrame:
    """
    Fetch all recently filed patents from QuiverQuant's live patent feed.
    """
    ...
    try:
        logger.info("📄 Fetching live patent filings from QuiverQuant")
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        data = response.json()
        return pd.DataFrame(data)
    except Exception as e:
        logger.error(f"❌ Failed to fetch live patent data: {e}")
        return pd.DataFrame()
