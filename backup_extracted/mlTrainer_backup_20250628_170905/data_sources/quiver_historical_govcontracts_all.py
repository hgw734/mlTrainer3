...
def fetch_all_historical_gov_contracts() -> pd.DataFrame:
    """
    Fetch all historical government contracts across all companies.
    """
    ...
    try:
        logger.info("🏛️ Fetching ALL historical government contracts (QuiverQuant)")
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        data = response.json()
        return pd.DataFrame(data)
    except Exception as e:
        logger.error(f"❌ QuiverQuant full govcontractsall fetch failed: {e}")
        return pd.DataFrame()
