...
def fetch_live_political_beta() -> pd.DataFrame:
    """
    Fetch live political beta sensitivity scores from QuiverQuant.
    """
    ...
    try:
        logger.info("🏛️ Fetching live political beta data from QuiverQuant")
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        data = response.json()
        return pd.DataFrame(data)
    except Exception as e:
        logger.error(f"❌ Failed to fetch political beta data: {e}")
        return pd.DataFrame()
