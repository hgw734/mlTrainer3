        ...
        if quote:
            ...
            print(f"  High/Low: ${quote.high:.2f}/${quote.low:.2f}")
                ...
                if hist:
                    ...
                    print(
                        f"  Date range: {hist.start_date.strftime('%Y-%m-%d')} to {hist.end_date.strftime('%Y-%m-%d')}")
                    print(
                        f"  Latest close: ${hist.data['close'].iloc[-1]:.2f}")
                    print(
                        f"  30-day avg volume: {hist.data['volume'].mean():,.0f}")
                    else:
                        ...
                        print(
                            f"  Rate limit active: {metrics['rate_limit_active']}")
                        ...
                        return True
                            ...
                            logger.error(
                                f"Polygon test error: {e}", exc_info=True)
                            return False
                            ...
                            def test_fred_connection():
                                ...
                                try:
                                    ...
                                    gdp = connector.get_series_data(
                                        "GDP", start_date="2023-01-01")
                                    if gdp:
                                        print(f"✓ GDP Data:")
                                        ...
                                        print(
                                            f"  Latest value: ${gdp.data['value'].iloc[-1]:,.2f} billion")
                                        else:
                                            ...
                                            unrate = connector.get_series_data(
                                                "UNRATE", start_date="2024-01-01")
                                            if unrate:
                                                print(f"✓ Unemployment Rate:")
                                                print(
                                                    f"  Series: {unrate.name}")
                                                print(
                                                    f"  Latest rate: {unrate.data['value'].iloc[-1]:.1f}%")
                                                print(
                                                    f"  3-month avg: {unrate.data['value'].tail(3).mean():.1f}%")
                                                else:
                                                    print(
                                                        "✗ Failed to get unemployment data")
                                                    ...
                                                    if results:
                                                        print(
                                                            f"✓ Found {len(results)} inflation-related series:")
                                                        for i, series in enumerate(
                                                                results[:3], 1):
                                                            print(
                                                                f"  {i}. {series['id']}: {series['title']}")
                                                            print(
                                                                f"     Frequency: {series['frequency']}, Units: {series['units']}")
                                                            else:
                                                                print(
                                                                ...
                                                                print(
                                                                    f"✓ Available popular series: {len(popular)}")
                                                                for code, name in list(
                                                                    ...
                                                                    return True
                                                                        ...
                                                                        logger.error(
                                                                            f"FRED test error: {e}", exc_info=True)
                                                                        return False
