...
class DataLineage:
    ...
    freshness_threshold: timedelta = field(default_factory=lambda: timedelta(hours=24))
    ...
    def add_calculation_step(self, operation: str, inputs: List['DataLineage'] = None):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'input_lineages': [lineage.to_dict() for lineage in self.input_lineages],
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'DataLineage':
        ...
        return cls(
            ...
            input_lineages=[cls.from_dict(lineage) for lineage in data['input_lineages']],
            freshness_threshold=timedelta(hours=data['freshness_threshold_hours']),
            verification_status=LineageStatus(data['verification_status']),
    ...
    def __array_wrap__(self, out_arr, context=None):
        ...
        if context is not None:
            ...
            new_lineage = DataLineage(
                ...
                original_data_hash=hashlib.sha256(out_arr.tobytes()).hexdigest(),
...
class TrackedDataFrame(pd.DataFrame):
    ...
    def __init__(self, data=None, lineage: DataLineage = None, *args, **kwargs):
    ...
    def _update_lineage(self, operation: str, inputs: List[DataLineage] = None):
        """Update lineage after operation"""
        new_lineage = DataLineage(
            source=DataSource.INTERNAL_CALCULATION,
            timestamp=datetime.now(),
            original_data_hash=hashlib.sha256(self.to_string().encode()).hexdigest(),
            calculation_path=[operation],
            input_lineages=inputs or [self.lineage]
        )
...
class DataLineageSystem:
    ...
    def _load_allowed_sources(self) -> Set[str]:
        ...
        if not allowed_sources:
            allowed_sources = {'polygon_api', 'fred_api', 'wikipedia', 'internal_calculation'}
            ...
            try:
                with open(registry_path, 'r') as f:
                    ...
                    for key, lineage_data in data.items():
                        self.lineage_registry[key] = DataLineage.from_dict(lineage_data)
                logger.info(f"Loaded {len(self.lineage_registry)} lineage records")
            except Exception as e:
    ...
    def _save_lineage_registry(self):
        ...
        try:
            with open(registry_path, 'w') as f:
                data = {key: lineage.to_dict() for key, lineage in self.lineage_registry.items()}
                json.dump(data, f, indent=2, default=str)
        except Exception as e:
            logger.error(f"Error saving lineage registry: {e}")
    
    def create_lineage(self, data: Any, source: DataSource, metadata: Dict[str, Any] = None) -> DataLineage:
        """Create new data lineage"""
        # Verify source is allowed
        if source.value not in self.allowed_sources:
            raise ValueError(f"Data source '{source.value}' is not in allowed sources: {self.allowed_sources}")
    ...
    def verify_lineage(self, lineage: DataLineage) -> bool:
        ...
        if lineage.source.value not in self.allowed_sources:
            logger.warning(f"Blocked data from unauthorized source: {lineage.source.value}")
            lineage.verification_status = LineageStatus.BLOCKED
            return False
        
        # Check if source is blocked
        if lineage.source.value in self.blocked_sources:
            logger.warning(f"Blocked data from blocked source: {lineage.source.value}")
            lineage.verification_status = LineageStatus.BLOCKED
            return False
    ...
    def _verify_calculation_step(self, step: str) -> bool:
        ...
        allowed_operations = {
            'add', 'subtract', 'multiply', 'divide', 'mean', 'std', 'sum',
            'momentum', 'volatility', 'correlation', 'regression', 'clustering',
            'normalize', 'standardize', 'filter', 'sort', 'rank'
        }
    ...
    def create_tracked_array(self, data: np.ndarray, source: DataSource, metadata: Dict[str, Any] = None) -> TrackedArray:
        """Create tracked NumPy array"""
        lineage = self.create_lineage(data, source, metadata)
        return TrackedArray(data, lineage)
    
    def create_tracked_dataframe(self, data: pd.DataFrame, source: DataSource, metadata: Dict[str, Any] = None) -> TrackedDataFrame:
        """Create tracked pandas DataFrame"""
        lineage = self.create_lineage(data, source, metadata)
        return TrackedDataFrame(data, lineage)
    
    def create_tracked_series(self, data: pd.Series, source: DataSource, metadata: Dict[str, Any] = None) -> TrackedSeries:
...
if __name__ == "__main__":
    ...
    print(f"Created tracked data with lineage: {tracked_data.lineage.get_full_lineage()}")
