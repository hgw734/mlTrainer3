...
def main():
    ...
    logger.info("\n📡 API CONFIGURATION:")
    logger.info(f"  Approved Sources: {[s.value for s in config.get_all_approved_sources()]}")
    logger.info(f"  Polygon API Valid: {config.validate_api_source('polygon')}")
    logger.info(f"  Invalid API Valid: {config.validate_api_source('invalid')}")
    ...
    logger.info(f"  Model Valid: {config.validate_ai_model_config('claude-3-5-sonnet')}")
    ...
    logger.info(f"  XGBoost Valid: {config.validate_mathematical_model_config('xgboost')}")
