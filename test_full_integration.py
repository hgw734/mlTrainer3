...
I recommend starting a trial on AAPL using an ensemble approach:
    ...
    for key, value in list(parsed["extracted_params"].items()):
        ...
        if trial_config:
            ...
            ml_feedback = {"type": "volatility_check", "volatility": 7.8, "train_ratio": 0.75}
            ...
            action_parsed = mlagent.parse_mltrainer_response(mltrainer_action_response)
            if "action" in action_parsed["extracted_params"]:
                action = action_parsed["extracted_params"]["action"]
                print(f"✅ Action detected: {action}")
                execution = mlagent.execute_action(action, action_parsed["extracted_params"])
                ...
                for file_path in files_to_check:
                    ...
                    print(f"{'✅' if exists else '❌'} {file_path.name}: {'exists' if exists else 'missing'}")
                    ...
                    print(f"✅ Formatted for mlTrainer: {len(goal_formatted)} chars")
