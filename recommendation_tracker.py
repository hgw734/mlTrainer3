...
class TradingRecommendation:
    ...
    def expected_return(self) -> float:
        ...
        loss_pct = ((self.stop_loss - self.entry_price) / self.entry_price) * 100
        return (gain_pct * self.profit_probability) + (loss_pct * (1 - self.profit_probability))
...
class RecommendationTracker:
    ...
    async def scan_for_opportunities(self, symbols: List[str]) -> List[TradingRecommendation]:
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            for symbol in symbols:
                future = executor.submit(self._analyze_symbol, symbol, market_conditions)
    ...
    def _analyze_symbol(self, symbol: str, market_conditions: Dict) -> List[TradingRecommendation]:
        ...
        try:
            ...
            for model_name in self.momentum_models:
                try:
                    ...
                    logger.error(f"Error running {model_name} on {symbol}: {e}")
                    continue
    ...
    def _create_recommendation(self, symbol: str, current_price: float, 
                             model_result: Any, model_name: str, 
                             timeframe: str, market_conditions: Dict) -> Optional[TradingRecommendation]:
        ...
        try:
            ...
            profit_probability = 0.5 + (signal_strength * 0.3) + (confidence * 0.2)
    ...
    def _filter_recommendations(self, recommendations: List[TradingRecommendation]) -> List[TradingRecommendation]:
    ...
    def _rank_recommendations(self, recommendations: List[TradingRecommendation]) -> List[TradingRecommendation]:
        ...
        for rec in recommendations:
            ...
            score = (
                ...
                rec.confidence * 0.2 +
                min(rec.risk_reward_ratio / 5, 1.0) * 0.2  # Normalize RR to 0-1
            )
    ...
    def _auto_paper_trade(self, recommendations: List[TradingRecommendation]):
        ...
        for rec in recommendations:
            try:
                ...
                position_data = {
                    'id': hash(f"{rec.symbol}{rec.timestamp}") % 1000000,  # Generate ID
                    'symbol': rec.symbol,
                ...
                if position:
                    logger.info(f"Opened paper trade for {rec.symbol} based on {rec.model_used}")
                    
            except Exception as e:
                logger.error(f"Error opening paper trade for {rec.symbol}: {e}")
    ...
    async def _get_market_conditions(self) -> Dict:
        ...
        try:
            ...
            if spy_data and len(spy_data) > 0:
                ...
                conditions['spy_trend'] = 'uptrend' if spy_returns.mean() > 0 else 'downtrend'
